...
参考実測(4 vCPU の QEMU ゲスト、バッキングは vm-ubuntu.sh の virtio データ
ディスク(cache=none、ホストページキャッシュなし)、fio 4k randwrite 4ジョブ。再現は VM 内で
[scripts/cpu-demo.sh](scripts/cpu-demo.sh) を root 実行):

| 構成 | デーモン合計CPU | IOPS |
|---|---|---|
| 1デバイス `-q 1`(既定) | ≈42%(キュースレッド1本) | 24.3k |
| 1デバイス `-q 4` | ≈91%(4コアに分散) | 27.8k |

この負荷では律速がバッキング側なので IOPS はキュー数でほぼ変わらない —
キューが増やすのは使える CPU で、効くのは CPU 律速のときだけ。
なおバッキングが O_DIRECT 非対応(例: tmpfs 上のファイル)だと io_uring が
バッファード IO を io-wq カーネルワーカーに punt し、iou-wrk-* スレッドが
現れて遅くもなる点に注意。
...
何をどの手法で守っているかの全体像は [docs/testing.md](docs/testing.md)。
...
./scripts/vm-ubuntu.sh stress    # ゲスト内でブロック層ストレス(fio --verify / ext4+fsck / dm-flakey)
...
- [docs/testing.md](docs/testing.md) — テスト戦略: 不変条件と、レイヤごとの検証手法(ユニット/カオス/ブロック層ストレス)
