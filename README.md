...
fio 4k randread 4ジョブ。再現は VM 内で [scripts/cpu-demo.sh](scripts/cpu-demo.sh) を root 実行):

| 構成 | デーモン合計CPU | IOPS |
|---|---|---|
| 1デバイス `-q 1`(既定) | ≈77%(キュースレッド1本) | 91.5k |
| 1デバイス `-q 4` | ≈213%(4コアに分散) | 185k |
| 2デバイス(各 `-q 1`) | ≈121%(別コアに分離) | 52k + 52k |
