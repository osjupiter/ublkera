...
**ゼロコピー**: カーネルが `UBLK_F_AUTO_BUF_REG`(6.16+)に対応していれば
自動で有効になる。リクエストのページがキューの io_uring に fixed buffer と
して直接登録され、IO バッファ経由のメモリコピーが消える(このとき IO バッファ
自体を確保しない)。`add` 応答の `"zero_copy"` で確認でき、`--no-zero-copy`
で無効化できる。非対応カーネルでは従来のコピー方式に自動フォールバック。
...
- ゼロコピー有効時(`"zero_copy": true`)は IO バッファをそもそも確保しない
  ので、この項は消えて era マップだけになる
