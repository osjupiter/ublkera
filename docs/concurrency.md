...
刻印は結果コードによらず**要求レンジ全体**に対して行う。失敗(EIO)や
部分書き込みでも媒体は途中まで変わっている可能性があり、成功分だけを
刻印すると過小報告になるため、「変わったかもしれない範囲」を常に dirty
扱いする(失敗したのに実際は何も変わっていなかった場合は過剰報告になる
だけで、消費者のコピーが増えるだけ)。DISCARD も同様に要求レンジ全体を
刻印する(discard はデータをゼロ化する = 変更なので、dm-era 同様
「changed」として記録する)。この性質は dm-flakey でバッキングの書き込みを
全部失敗させるテスト(scripts/blkstress.sh パート3)で検証している。
...
- 上記の保証は**地図(era マップ)の取りこぼしがない**ことであって、
  コピーした**像の一貫性**ではない。IO を流したまま dump の範囲を読むと、
  ブロックごとに読んだ瞬間が違う **fuzzy image** になる(これは CBT 固有
  ではなく、スナップショット無しのライブブロックコピー全般の性質)。
  復元後に fsck / ジャーナル回復を前提にするなら実用上は足りることが多い
- 点整合(point-in-time)の像が必要なら、freeze → checkpoint だけでは
  不十分で、**最後の差分コピーの間だけ凍結を維持する**反復収束の手順を
  使う(具体的な手順は [dump-format.md](dump-format.md))。アプリ整合が
  必要なら freeze のタイミングをアプリの静止点に合わせる(dm-era でも同じ)
