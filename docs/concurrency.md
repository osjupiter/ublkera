...
**刻印が提出前ではなく完了後である理由**: 提出から完了までの間、
「媒体は変わり始めているのにマップは未刻印」という窓が存在する。この窓で
checkpoint + dump が走って chunk を読み飛ばしても、完了時の刻印は §3 の
リトライにより「その時点までに完了したどの checkpoint よりも新しい era」に
なるため、**必ず次回の差分に現れる**(完了が checkpoint より前だった場合は
データも先に着地しているので今回の差分とコピーが拾う — どちらに転んでも
消えない)。完了しないままデーモンが死んだ場合は unclean 検知 → 全面 dirty
が受け止める。dm-era のように提出前に刻印してもクラッシュ時にメモリ上の
マップごと消える事情は変わらず、増えるのは過剰報告だけなので、単純な
完了時刻印を採っている。
